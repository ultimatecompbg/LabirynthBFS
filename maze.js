...
function bfs(maze) {
    ...
    while(queue.length != 0) {
        ...
        tryPosition({row:position.row+1, column:position.column, path:position.path});
        tryPosition({row:position.row-1, column:position.column, path:position.path});
        tryPosition({row:position.row, column:position.column+1, path:position.path});
        tryPosition({row:position.row, column:position.column-1, path:position.path});
    }
    if(!exitfound) {
...
function isInsideMaze(position) {
    if(position.row >= 0 && position.row < maze.length && position.column  >= 0 && position.column < maze[0].length) {
       return true;
	}
}
